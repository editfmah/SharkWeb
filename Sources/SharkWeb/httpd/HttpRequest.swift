...
public enum InternalHandlingError {
    case none
    case tooLarge
}
...
public class HttpRequest {
    ...
    public var internalError: InternalHandlingError = .none
    ...
    public func parseUrlencodedForm() -> [String:String] {
        ...
        for t in parseUrlencodedFormTuple() {
            retVal[t.0] = t.1
        }
        return retVal
    }
    
    public func parseUrlencodedFormTuple() -> [(String, String)] {
    ...
    internal func cookieData() -> [String:String] {
        
        var retVal: [String:String] = [:]
        
        if headers["cookie"] != nil {
            for c in headers["cookie"]!.components(separatedBy: ";") {
                let kv = c.components(separatedBy: "=")
                if kv.count > 0 {
                    retVal[kv[0]] = ""
                }
                if kv.count > 1 {
    ...
    public struct MultiPart {
        ...
        public var type: String? {
            return headers["content-type"]?.unquote()
        }
