...
//    var thisUser: SWUser?
//    var thisCookie: SWCookie
//    var thisNavigation: NavigationPosition
//    var scope: Closure
//    var ret: HttpResponse?
//    var semaphore: DispatchSemaphore
//    init(cookie: SWCookie, nav: NavigationPosition, scp: @escaping Closure, sem: DispatchSemaphore) {
//        thisCookie = cookie
//        if let tUsr = SWAuthenticator().validate(cookie.authToken ?? "") {
//            thisUser = tUsr
//        }
//        thisNavigation = nav
//        self.scope = scp
...
//                scopesBuffer[threadId] = ""
//                if let user = self.thisUser {
//                    userBuffer[threadId] = user
//                } else {
//                    userBuffer[threadId] = nil
//                }
//                navigationBuffer[threadId] = self.thisNavigation
//                user = userBuffer[threadId]
//                navigation = self.thisNavigation
//                self.scope()
//                var headers: [String:String] = [:]
//                headers["Content-Type"] = "text/html"
//                if user?.token != nil {
//                    headers["Set-Cookie"] = "\(self.thisCookie.compiled()) path=/;"
//                } else {
//                    headers["Set-Cookie"] = "AuthToken=; path=/;"
//                }
//                self.ret = .raw(200, "OK", headers, {
//                    try? $0.write([UInt8](("<!DOCTYPE html>"  + (scopesBuffer[threadId] ?? "")).utf8))
//                    // clean up the stack space now
//                    scopesBuffer.removeValue(forKey: threadId)
//                    navigationBuffer.removeValue(forKey: threadId)
//                    userBuffer.removeValue(forKey: threadId)
//                })
...
//}
//
//public func scopes(cookie: SWCookie, thisNavigation: NavigationPosition, _ scope: @escaping Closure) -> ((HttpRequest) -> HttpResponse) {
//    return { _ in
//        
//        let semaphore = DispatchSemaphore(value: 0)
//        let executionObject = scopeExecution(cookie: cookie, nav: thisNavigation, scp: scope, sem: semaphore)
//        executionObject.start()
//        semaphore.wait()
//        return executionObject.ret!
//        
//    }
//}
//
...
//
//public var user: SWUser?
//public var context: Mutex?
//public var navigation: NavigationPosition!
//
...
//
//public func a(_ closure: Closure) {
//    element("a", closure)
//}
...
//    evaluate(node, [:], closure)
//}
//public func element(_ node: String, _ attrs: [String: String?] = [:], _ closure: Closure) { evaluate(node, attrs, closure) }
//
//public var scopesBuffer = [String: String]()
//public var userBuffer = [String: SWUser]()
//public var navigationBuffer = [String: NavigationPosition]()
//
//// swiftlint:disable cyclomatic_complexity function_body_length
//public func evaluate(_ node: String, _ attrs: [String: String?] = [:], _ closure: Closure) {
//
//    // Push the attributes.
//
//    let stackid = idd
...
//    acceptCharset = nil
//    inner = nil
//    user = userBuffer["\(Thread.current)"]
//    navigation = navigationBuffer["\(Thread.current)"]
//
//    scopesBuffer["\(Thread.current)"] = (scopesBuffer["\(Thread.current)"] ?? "") + "<" + node
//
//    // Save the current output before the nested scope evalutation.
//
//    var output = scopesBuffer["\(Thread.current)"] ?? ""
//
//    // Clear the output buffer for the evalutation.
//
//    scopesBuffer["\(Thread.current)"] = ""
//
...
//
//    if let inner = inner {
//        scopesBuffer["\(Thread.current)"] = output + ">" + (inner) + "</" + node + ">"
//    } else {
//        let current = scopesBuffer["\(Thread.current)"]  ?? ""
//        scopesBuffer["\(Thread.current)"] = output + ">" + current + "</" + node + ">"
//    }
//
//    // Pop the attributes.
//    idd = stackid
